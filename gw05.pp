class { 'ffnord::params':
  router_id => "10.71.9.1",  # The id of this router, probably the ipv4 address
...
ffnord::mesh { 'mesh_ffnh':
  ...
  , mesh_code => "ffnh"
  , mesh_as => "64889"
  , mesh_mac  => "fe:ed:be:ef:ff:05"
  , vpn_mac  => "fe:ed:be:ff:ff:05"
  , mesh_ipv6 => "fd8f:14c7:d318::ff05/64"
  , mesh_ipv4  => "10.71.9.1/19"        # ipv4 address of mesh device in cidr notation, e.g. 10.35.0.1/19
  ...
  , fastd_secret => "/root/05gw-fastd-secret.key"
  , fastd_port   => 10050
  ...
  , dhcp_ranges => ['10.71.9.2 10.71.10.254']    # the whole net is 10.71.0.0 - 10.71.63.255
                                                # so take one 32rd of this range but don't give out the ip of the gw itself
  , dns_servers => ['10.71.9.1']                # should be the same as $router_id
}
